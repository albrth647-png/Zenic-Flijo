...
def bootstrap_hat(
    ...
    specialists_by_domain: dict[str, dict[str, Any]] = {
        "operaciones": {
...
def get_hat_router(event_bus: Any = None) -> Any:
    """Factory para obtener el HATRouter singleton.

    En producción, esto debe llamarse UNA sola vez al startup.
    En tests, se puede llamar con force=True para resetear.

    Returns:
        HATRouter inicializado.
    """
    global _cached_router
    if "_cached_router" not in globals() or _cached_router is None:
        _cached_router = bootstrap_hat(event_bus=event_bus)
    return _cached_router


_cached_router: Any | None = None
